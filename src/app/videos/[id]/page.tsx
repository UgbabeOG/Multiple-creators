...
export async function generateMetadata(
  ...
  return {
    ...
    description:
      video.initialDescription || `Details for the music video ${video.title}.`,
  };
}
...
export default async function VideoDetailsPage({
  ...
  const videoHints = {
    "fine-wine": "galaxy space",
    "its-ok": "neon city",
    yourside: "flower timelapse",
    "2mins": "city street",
  };
  const dataAiHint =
    videoHints[video.id as keyof typeof videoHints] || "video still";
  ...
  return (
    <div className="space-y-8">
      ...
      {video.artist && (
        <p className="text-xl text-muted-foreground -mt-6">By {video.artist}</p>
      )}
